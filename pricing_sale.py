...
def def_pricing_sale(prices_in_vesta, art_dubl_df, prices_LT_sale, file_directory_prices_in_vesta, todays_date):
    ...
    # prices_LT_concat.to_excel(file_directory_prices_in_vesta + 'Файл ЦО промежуточный_sale.xlsx')
    ...
    # Если есть расхождения в ценах: Базовый(РФ)-Уч.цена вал., Розница ЭКС-Розница
    ...
    for i in range(len(prices_LT_merge)):
        if prices_LT_merge.iloc[i,3] != prices_LT_merge.iloc[i,6] \
                or prices_LT_merge.iloc[i,4] / prices_LT_merge.iloc[i,7] > 1.01\
                or prices_LT_merge.iloc[i,4] / prices_LT_merge.iloc[i,7] < 0.99:
    ...
    if len(nomenclature_change_sale) > 0:
        nomenclature_change_sale.to_excel(file_directory_prices_in_vesta + 'Отчёт по изменениям_sale_' + todays_date + '.xlsx')
